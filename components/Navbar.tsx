...
export default function Navbar() {
    ...
    useEffect(() => {
        ...
        setIsAuthenticated(!!token); // Simple check for token presence
    }, []);
        ...
        <header className="grid grid-cols-[1fr-1fr] md:grid-cols-3 items-center border-b border-neutral-200 bg-white">
            ...
            <div className="flex justify-center items-center gap-4">
                {!isAuthenticated ? (
                    <Link
                        href="/login"
                        className="flex items-center gap-2 text-slate-700 hover:text-cyan-600 hover:bg-slate-100 transition-colors px-4 py-2 rounded-lg cursor-pointer"
                    >
                        <svg
                            xmlns="http://www.w3.org/2000/svg"
                            width="20"
